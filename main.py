...
# --- إعدادات سيرفر الويب (Flask) لمنع النوم على Render ---
...
def index():
  return (
      "<h1>Bot is active and running successfully!</h1><p>Flask web server is"
      " online for Render and UptimeRobot.</p>"
  )
...
# تشغيل سيرفر الويب في خلفية النظام بالتوازي مع البوت
...
MONGO_URI = "mongodb+srv://sap786118_db_user:كلمة_المرور_هنا@cluster0.extvh0l.mongodb.net/?appName=Cluster0"
...
def load_data():
  ...
  for uid, udata in data.items():
    ...
    if isinstance(udata, dict):
      if "accounts" not in udata:
        udata["accounts"] = []
      if "texts" not in udata:
        udata["texts"] = []
      if "groups" not in udata:
        udata["groups"] = []
      if "banned" not in udata:
        udata["banned"] = False
  return data
...
def main_menu(is_admin_user=False):
  ...
  if is_admin_user:
    keyboard.insert(
        0, [InlineKeyboardButton("🛠️ لوحة تحكم المطور", callback_data="admin_panel")]
    )
...
async def start_command(client, message):
  ...
  if not await is_subscribed(client, message.from_user.id):
    await message.reply_text(
        "❌ عذراً، يجب عليك الاشتراك في قناة البوت أولاً لتتمكن من"
  ...
  if user_id not in data:
    ...
    if "accounts" not in data[user_id]:
      data[user_id]["accounts"] = []
    if "texts" not in data[user_id]:
      data[user_id]["texts"] = []
    if "groups" not in data[user_id]:
      data[user_id]["groups"] = []
    if "banned" not in data[user_id]:
      data[user_id]["banned"] = False

  data[user_id]["state"] = None
  save_data(data)

  welcome_template = data.get("_settings", {}).get(
      "welcome_message",
      "أهلاً بك يا {name} في بوت النشر التلقائي.\n\nاستخدم الأزرار أدناه للتحكم"
      " بحساباتك، مجموعاتك، وإدارة عمليات النشر بكل سهولة.",
  )
...
async def callback_handler(client, call):
  ...
  if call.data == "back_main":
    ...
    admin_kb = InlineKeyboardMarkup([
        [
            InlineKeyboardButton(
                "📊 إدارة المشتركين", callback_data="admin_member_count"
            ),
    ...
    await call.message.edit_text(
        "👑 **مرحباً بك في لوحة تحكم الأدمن الخاصة:**", reply_markup=admin_kb
    )
    ...
    save_data(data)
    await call.message.edit_text(
        "📢 أرسل الآن الرسالة (نص، صورة، فيديو...) التي تريد إذاعتها لجميع الأعضاء:",
    ...
    current_welcome = settings.get(
        "welcome_message", "أهلاً بك يا {name} في بوت النشر التلقائي."
    )
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                "➕ إضافة رسالة أو صورة جديدة", callback_data="add_text"
            )
        ],
    ...
    save_data(data)
    await call.message.edit_text(
        "✍️ أرسل الآن **نص** أو **صورة** (مع الوصف إن أردت) لتكون ضمن رسائل النشر:",
        reply_markup=back_menu(),
    )
    ...
    save_data(data)
    await call.message.edit_text(
        "🗑️ تم حذف جميع الرسائل والصور.", reply_markup=back_menu()
    )
    ...
    if (
      ...
      await call.answer(
          "❌ يجب إضافة حساب، ورسالة أو صورة، ومجموعة واحدة على الأقل أولاً!",
          show_alert=True,
      )
    else:
...
async def message_handler(client, message):
  ...
  if state == "waiting_for_admin_broadcast":
    ...
    status_msg = await message.reply_text(
        "⏳ جاري بدء الإذاعة لجميع المشتركين..."
    )
    all_users = load_data()
    ...
    await message.reply_text(
        msg_result, reply_markup=main_menu(admin_status)
    )
    return
    ...
    try:
      ...
      save_data(data)
      await message.reply_text(
          "🔐 الحساب محمي بكلمة مرور. أرسلها الآن:"
      )
    except Exception as e:
    ...
    try:
      ...
      save_data(data)
      await message.reply_text(
          reply_text, reply_markup=main_menu(admin_status)
      )
    except Exception as e:
      await message.reply_text(f"❌ حدث خطأ: {e}")
      data[user_id]["state"] = None
    ...
    try:
      ...
      save_data(data)
      await message.reply_text(
          "✅ تم ضبط الوقت بنجاح.", reply_markup=main_menu(admin_status)
      )
    except Exception as e:
...
async def main():
  # بدء مهام الخلفية للنشر التلقائي
  asyncio.create_task(background_publisher())

  await app.start()
  print("البوت وسيرفر فلاسك يعملان الآن بنجاح تام...")
